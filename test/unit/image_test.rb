...
class ImageTest < ActiveSupport::TestCase
  def setup
    @image = Image.find images(:login).id
  end

  test 'find' do
    assert_kind_of Image, @image
    assert_equal images(:login).name, @image.name
    assert_equal images(:login).caption, @image.caption
    assert_equal images(:login).image_file_name, @image.image_file_name
    assert_equal images(:login).image_content_type, @image.image_content_type
    assert_equal images(:login).image_file_size, @image.image_file_size
    assert_equal images(:login).image_updated_at, @image.image_updated_at
  end
  ...
  test 'validates duplicated attributes' do
    @image.name = images(:welcome).name
    assert @image.invalid?
